...
export class WorktreeBuilder {
	...
	constructor(repositoryName: string, repositoryPath: string, worktreePath: string, branch: string) {
		this.worktree = {
			repositoryName,
			repositoryPath,
			worktreePath,
...
export function createTestEnvironment(vol: Volume, homeDir = '/home/testuser') {
	...
	vol.writeFileSync(
		path.join(groveFolder, 'repositories.json'),
	...
	vol.writeFileSync(
		path.join(groveFolder, 'recent.json'),
		JSON.stringify({ selections: [] }, null, '\t')
	);
